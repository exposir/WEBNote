...
- 解决方法： 添加一个**border.css**库，将利用**scroll缩放的原理**将边框重置。当我们需要使用一像素边框时只需要在标签上添加对应类名，如设置底部一像素边框就在标签上加入"border-bottom"的class名

#### 300毫秒点击延迟问题和点击穿透问题
在移动端开发中，某些机型上使用click事件会延迟300ms才执行，这样影响了用户体验。 
- 解决方法： 引入[fastclick.js](https://segmentfault.com/a/1190000015154809?utm_source=tag-newest)。
